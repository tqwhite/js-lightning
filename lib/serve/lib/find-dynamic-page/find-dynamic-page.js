...
const moduleFunction = function({ userConfiguration }) {
	...
	const get = (req, callback) => {
		...
		// A request path ending in '/' is a directory request — including the
		// site root '/'. For any such request, generate candidates by applying
		// defaultPageNameList within that directory. Previously this was only
		// done for the root, which meant /foo/ could never find /foo/index.js
		// and would silently fall through to the docRoot-module fallback
		// (aka 'HELLO WORLD' when the docRoot's package.json points to a
		// placeholder main).
		const isDirRequest = req.path.endsWith('/');

		const cancelDump = !userConfiguration.verbose;
		const finalPath = (isDirRequest ? defaultPageNameList.map(name => req.path + name) : [])
			.qtPassThrough(entire => {!isDirRequest && entire.push(req.path)})
			.qtPassThrough( entire => {!isDirRequest && entire.push(
				path.extname(req.path) == '.js' ? req.path :
				path.extname(req.path) == '.md' ? req.path :
				`${req.path}.js`
			)} )
			.qtPassThrough( entire => {!isDirRequest && entire.push( `${req.path}.md` )} )
			...
			.qtLast( (req.path == '/' && isModule(userConfiguration.docRootPath)) ? userConfiguration.docRootPath : void 0 );
