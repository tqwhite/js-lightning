...
const moduleFunction = function({ userConfiguration }) {
	...
	const get = (req, callback) => {
		...
		const stripExcessDocRoot=(item, inx, all)=>(all.length<2 || !item.match(/^.*docRoot\/$/))
		...
		finalPath.qtDump({label:'finalPath', cancelDump}); //qtDump() doesn't pass that final value, nasty bug
