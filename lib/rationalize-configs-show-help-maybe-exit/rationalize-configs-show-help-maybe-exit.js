...
const moduleFunction = function(args = {}) {
	...
	const defaultConfigurationValues=configFileProcessor.getConfig(path.join(__dirname, 'assets', 'systemParameters_defaults.ini'));
	defaultConfigurationValues._meta.contributors=[];
	defaultConfigurationValues._meta.contributors=([].qtPush(defaultConfigurationValues._meta.qtClone()));
	...
	if (commandLineParameters.qtGetSurePath('values.defaultPageNameList')){
		if (defaultConfigurationValues.qtGetSurePath('commandLineParameters.values.defaultPageNameList')){
			delete defaultConfigurationValues.commandLineParameters.values.defaultPageNameList;
		}
	}
	...
	const defaultPageNameList=finalParameterSet.qtGetSurePath('values.defaultPageNameList', []);

	
	return { siteDirectory, docRootPath, port, noServe, generateControlFiles, domain, defaultPageNameList };
};
